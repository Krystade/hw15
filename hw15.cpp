...
// Q#1
// An STL container is a templated class that is used to hold other types of objects
// An iterator is a generalization of a pointer. 5 different types, (input, output, forward, bidirectional, and random access).
// An algorithm is a templated function that manipulates data in a container. Always has atleast 2 arguments

// Q#2
// Because of the fact that algorithms are templates that are paramaterized by the type of iterator, the algorithm is not restricted to a single type of container. This decoupling
// of the algorithm makes it much more versatile.

// Q#5
// With the initial value of l being -1, if there is not any value less than -1 then the algorithm will not be able to find the lowest value.
